...
contract HdexRouter is IHdexRouter {
    ...
    function addLiquidityETH(
        ...
        address pair = HdexLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = HdexLibrary.pairFor(factory, tokenA, tokenB);
        IHdexPair(pair).transferFrom(msg.sender, pair, liquidity);
    ...
    {
        require(path[0] == WETH, 'HdexRouter: INVALID_PATH');
        amounts = HdexLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'HdexRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(HdexLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'HdexRouter: INVALID_PATH');
        amounts = HdexLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'HdexRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, HdexLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(HdexLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
